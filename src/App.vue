  ...
  <div class="min-h-screen bg-gray-50 flex flex-col">
    ...
    <el-dialog v-model="showHelp" title="使用说明" width="650px">
      <div class="space-y-4 text-sm text-gray-600">
        <div>
          <h3 class="font-medium text-gray-800 mb-1">1. 设置 API Key 与 Base URL</h3>
          <p>点击右上角「API 设置」按钮，输入 API Key 并选择对应的 Base URL。普通 API Key（sk-xxx）搭配「普通 API」；Token Plan Key（tp-xxx）搭配对应集群。</p>
        </div>
        <div>
          <h3 class="font-medium text-gray-800 mb-1">2. 选择模型与合成模式</h3>
          <ul class="list-disc list-inside space-y-1">
            <li><strong>MiMo-V2.5-TTS</strong>：内置精品音色，开箱即用</li>
            <li><strong>MiMo-V2.5-TTS-VoiceDesign</strong>：通过文本描述创造自定义音色</li>
            <li><strong>MiMo-V2.5-TTS-VoiceClone</strong>：上传音频样本，复刻任意音色</li>
          </ul>
        </div>
        <div>
        ...
        <div>
          ...
          <p>输入文本后点击「生成语音」，等待生成完成后可在线播放，手动点击「下载」保存音频文件。</p>
        </div>
        <el-divider />
...
function onPlayHistory(item: TTSHistoryItem) {
  ...
  if (item.audioBase64) {
    const mimeType = item.audioFormat === 'mp3' ? 'audio/mpeg' : 'audio/wav'
